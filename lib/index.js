...
const rateChord = (notes, chord) => {
    ...
    forEach(parts, part => {
        if (!includes(notes, part)) {
            ...
            // we don't care if the 0 is missing out
            if (part === 0) sum += 0;
...
const getCadenseBonuses = (cadenses, current) => {
    ...
    forEach(cadenses, ([chord1, chord2, diff, reversible = true, score = 2]) => {
        let other = null;
        if (current === chord1) {
            other = chord2;
        }
        else if (current === chord2 && reversible) {
            other = chord1;
            diff  = -diff;
        }
        else return;
        bonuses.push({current, other, diff: wrapAround(diff), score});
    });
...
const applyCadenseBonuses = (currentChord, cadenses, possibilities) => {
    ...
    forEach(possibilities, possibility => {
        ...
        if (bonus) {
            ...
            possibility.cadense      = bonus;
            possibility.cadenseBonus = bonus.score;
        }
    });
...
const getNextChord = (currentChord = null, cadenses, chords, notes, octaves = 1) => {
    ...
    possibilities = map(possibilities, possibility => {
        possibility.rating = possibility.rating + Math.random();
        return possibility;
    });

    const nextChord = maxBy(possibilities, ({rating}) => rating);

    return {nextChord, possibilities};
};
